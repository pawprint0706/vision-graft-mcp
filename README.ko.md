...
아래 블록을 하나씩 터미널에 붙여넣으세요.

```bash
# 1) 프로젝트 폴더로 이동 (경로는 실제 위치로 바꾸세요)
cd ~/Projects/vision-graft-mcp

# 2) 독립된 파이썬 환경 생성 (깔끔하게 관리됨)
python3 -m venv .venv
...
설치는 이걸로 끝입니다. (아래에서 `.venv/bin/` 접두사는 "이 폴더에 설치한
버전을 쓰라"는 뜻일 뿐입니다.)

> **팁:** 매번 `.venv/bin/`을 입력하기 싫다면, 터미널 창마다 한 번
> `source .venv/bin/activate`를 실행하세요. 그러면 `.venv/bin/vgmcp` 대신
> `vgmcp`만 입력하면 됩니다. 이 문서는 어디서나 작동하도록 전체 접두사를
> 유지합니다.
...
고른 제공자를 등록합니다. `--key -`를 쓰면 터미널이 **가려진 입력**으로 키를
물어보므로, 비밀이 화면이나 기록에 남지 않습니다.
