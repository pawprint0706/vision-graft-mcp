...
```
[아이콘: 상태 색상]
├─ 상태: 정상 / 조치 필요   ← 클릭 시 환경 재점검 + 항목별 결과 다이얼로그
├─ ─────────
├─ 모니터 캡쳐
│  └─ (서브메뉴: 모니터 0, 모니터 1, ...)
├─ 앱 창 선택 캡쳐
│  └─ (서브메뉴: 열려있는 창 목록)
├─ 영역 선택 캡쳐 (드래그)   ← §6.5 region_interactive
├─ 이미지 파일 열기
├─ ─────────
├─ 최근 이미지
...
├─ 설정
│  ├─ 타겟 폴더 설정...
│  ├─ 셀프 분석 모드 사용 (토글, 활성화 확인 후 비전 백엔드 호출 금지)
│  ├─ 비전 백엔드 관리... (Anthropic/OpenAI/OpenRouter/커스텀/Ollama 추가·수정·삭제, 기본값 지정)
│  ├─ 클립보드 프롬프트 템플릿 편집...
│  ├─ 자동 클립보드 복사 (토글)
│  ├─ 이미지 열기 시 타겟 폴더로 복사 (토글)
│  └─ 로그인 시 자동 시작 (토글, LaunchAgent)
└─ 종료
```
> 상태 항목이 최상단에서 환경 재점검을 겸하므로 설정 하위의 "환경 재검사"는 제거되었다.
> 셀프 분석 모드가 켜지면 비전 백엔드 자격증명 검사를 건너뛰고, 마지막 이미지 분석
> 메뉴를 비활성화하며, MCP 호출에는 검증 코드 없이 이미지를 직접 반환한다. 활성화 후
> 시작되는 백엔드 호출과 재시도는 차단하며, 활성화 전에 이미 시작된 요청은 완료될 수 있다.
...
- 선택한 파일을 타겟 폴더로 복사(또는 원본 경로 그대로 참조)하여 통합 파이프라인에 투입. **`copy_original` 설정으로 제어(기본 복사)하며, 트레이 설정의 "이미지 열기 시 타겟 폴더로 복사" 토글로도 노출.** 복사 시 파일명은 `opened_{원본명}_{타임스탬프}.png`(중복 방지). 이미 타겟 폴더 안의 파일이면 복사를 생략하고 원본을 참조.
...
### 4.3 상태 표시
아이콘은 단일 SVG(`assets/camera.svg`, 카메라)에서 생성한다. **정상**은 검정 **템플릿 이미지**로 두어 macOS가 라이트/다크 메뉴바에 맞춰 흑/백을 자동 적용하고, **노랑/빨강**은 각각 호박색/빨강으로 표시한다. 렌더 해상도는 `icon_size`로 조절(메뉴바 표시는 ~20pt). 생성물은 `~/.config/vgmcp/icons/`에 캐시.
