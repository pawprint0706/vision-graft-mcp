...
### 4.3 상태 표시
아이콘은 단일 라인아트 SVG(`assets/aperture.svg`, 카메라 조리개)에서 생성한다. **정상**은 검정 **템플릿 이미지**로 두어 macOS가 라이트/다크 메뉴바에 맞춰 흑/백을 자동 적용하고, **노랑/빨강**은 각각 호박색/빨강 선의 컬러 이미지로 표시한다. 렌더 해상도는 `icon_size`로 조절(메뉴바 표시는 ~20pt). 생성물은 `~/.config/vgmcp/icons/`에 캐시.
